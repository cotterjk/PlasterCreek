...
$('#query').click(function() { //TODO: Shift to on page load, query streamflow and animate from result
	$.ajax({
	  ...
	  success: function(json){
           //MAPPING MATH TODO
           // Discharge range: 7–1800 (undo exponential scale first???)
           // wavecolor: gradient 0–100%
           // waveheight: 10%–100%
           // wavespeed: 5000–400 (smaller for higher discharges)
           ...
           waveheight_str = json.value.timeSeries[0].values[0].value[0].value*100 + "px"
		   animateWave(wavespeed, wavecolor, waveheight_str);
		 },
	  error : function(XMLHttpRequest, textStatus, errorThrown) {
           $('#discharge-reading').text('unknown');
		   wavecolor = "rgb(87, 75, 99)";
           waveheight_str = '100px';
		   animateWave(100, wavecolor, waveheight_str);
	  }
	});
});
...
function animateWave(wavespeed, wavecolor, waveheight_str) {
